...
# =========================================================================
# MOTOR DEL EXPERIMENTO (Una partida entera)
# =========================================================================
def run_trial(p_informed, seed, max_time=3000.0):
    ...
    while box2d_time < max_time:
        if load_body.position.x > SLIT2_X + 1.0:
            success = True
            break
    ...
    return {
        'Seed': seed,
        'P_Informed': str(round(p_informed, 2)).replace('.', ','), # Cambiamos también aquí para Excel Español
...
if __name__ == "__main__":
    ...
    # ARREGLO DE CARPETA: Ruta absoluta basada en dónde está este script (src)
    ...
    for p in p_values:
        ...
        for trial in range(trials_per_p):
            ...
            # ARREGLO DE TIEMPO: 3000 segundos de simulación para que puedan llegar
    ...
    # ARREGLO DE EXCEL: Usamos punto y coma (delimiter=';')
    ...
    print(f"\n--- EXPERIMENTO COMPLETADO EN {round(total_time, 1)} SEGUNDOS REALES ---")
    print(f"Datos guardados de forma segura y perfectos para Excel en:")
    print(f"-> {csv_file}")