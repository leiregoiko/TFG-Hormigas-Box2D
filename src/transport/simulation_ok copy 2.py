...
def target_for_site(world_pos):
    ...
    # ¡ARREGLO PARA SINCRONIZAR 100% CON EL CIEGO!
...
def run_simulation():
    ...
    pygame.display.set_caption("TFG Leire — Replay Sincronizado Notarial")
    ...
    while running:
        ...
        if not paused or step_once:
            ...
            # --- AUTO-PAUSA: Se congela EXACTAMENTE en la misma línea que el ciego ---
            if not victory_recorded and load.body.position.x > SLIT2_X + 1.0:
        ...
        if victory_recorded:
            hud = [
                f"¡VICTORIA! Simulador Pausado.",
                f"TIEMPO EXACTO: {box2d_time:.1f} s",
                f"La cruz de la T ha pasado la meta (Igual que en el CSV)."
            ]
            color_text = (0, 150, 0)
        else:
