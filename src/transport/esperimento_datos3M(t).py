...
def run_trial(p_informed, seed, max_time=3000.0):
    ...
    while box2d_time < max_time:
        ...
        active_ants = [s for s in sites if s['state'] in ('info', 'puller')]
        
        # --- CÁLCULO DEL PARÁMETRO DE ORDEN M(t) ---
        if len(active_ants) > 0:
            sum_px = sum(s['pull_dir'].x for s in active_ants)
            sum_py = sum(s['pull_dir'].y for s in active_ants)
            m_t = math.hypot(sum_px, sum_py) / len(active_ants)
        else:
            m_t = 0.0
            
        sum_order_parameter += m_t

        for s in active_ants:
            wp = load_body.GetWorldPoint(s['local_pos'])
            wd = load_body.GetWorldVector(s['pull_dir'])
    ...
    return {
        ...
        'M_Medio_Orden': str(round(sum_order_parameter / steps, 3)).replace('.', ',') if steps > 0 else "0"
    }

# =========================================================================
# LANZADOR DEL EXPERIMENTO (CON GUARDADO EN TIEMPO REAL Y ESCUDO)
# =========================================================================
if __name__ == "__main__":
    ...
    columnas = ['Seed', 'P_Informed', 'Exito', 'Tiempo_Segundos', 'N_att_Medio', 'V_x_Media_cm_s', 'M_Medio_Orden']

    print("Abriendo el archivo Excel para el guardado en tiempo real...")
        ...
        try:
            for p in p_values:
                ...
                for trial in range(trials_per_p):
                    ...
                    print(f"Resultado: {estado} en {res['Tiempo_Segundos']} s | M={res['M_Medio_Orden']}")

        except KeyboardInterrupt:
            print("\n\n[!] ATENCIÓN: Simulación interrumpida manualmente por el usuario (Ctrl+C).")
            print(f"[✓] No te preocupes. Todas las partidas terminadas están a salvo en tu Excel.")

    print(f"\n--- EXPERIMENTO FINALIZADO EN {round(time.time() - start_time, 1)} SEGUNDOS REALES ---")
    print(f"Datos asegurados en: {csv_file}")